...
def nodeModel(nodeSet: NodeSet, h, eclipseLength, period):
    ...
    for i in range(numNodes):
        res_self_mat_inv = (
            nodeArray[i].opticalProperties.emissivity
            * nodeArray[i].physicalProperties.surfaceArea
            / (1 - nodeArray[i].opticalProperties.emissivity)
        )
        for j in range(numNodes):  # 1/Rij = A1 * Fij, skew symmetric matrix
            res_other_mat_inv = (
                nodeArray[i].physicalProperties.surfaceArea
                * nodeArray.radiationViewFactorMatrix[i][j]
            )
    ...
    for i in time:
        ...
        for n in range(len(nodeArray)):
            ...
            dQEmitted = emissivity * STEFAN_BOLTZMAN * surfaceArea * pow(temperature, 4)
            ...
            for x in range(numNodes):
                Ematrix.append(
                    -(STEFAN_BOLTZMAN * emissivity * pow(temperature, 4))
                    * res_self_mat_inv[x]
                )
            Jmatrix = np.linalg.solve(np.asarray(res_J_coeff), np.asarray(Ematrix))

            for y in range(numNodes):
                dQradiationNetwork += (Jmatrix[n] - Jmatrix[y]) * (
                    res_other_mat_inv[n][y]
                )
