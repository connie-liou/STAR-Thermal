...
@app.callback([Output('node-table', 'columns'), Output('radiation-table', 'columns'),
              ...
              [Input('num-nodes', 'value'), Input('radiation-table', 'data_timestamp'),
               Input('conductance-table', 'data_timestamp')],
    ...
    if numNodes == prevNumNodes:
        ...
        if numNodes >= len(nodeCols):
            ...
            for i in range(extra):
                ...
                newRow = {**{radCols[0]['id']: ('Surface ' + str(len(nodeCols) - 1))},
                          **{i['id']: 0 for i in radCols[1:]}}
                radRows.append(newRow)
                condCols.append({
                    'id': ('conductance-' + str(len(nodeCols) - 1)), 'name': ('Surface ' + str(len(nodeCols) - 1)),
                    'renamable': True, 'deletable': False, 'editable': True, 'type': 'numeric'
                })
                newRow = {**{condCols[0]['id']: ('Surface ' + str(len(nodeCols) - 1))},
                          **{i['id']: 0 for i in condCols[1:]}}
...
@app.callback(
    [Output('all-tab-container', 'className'), Output('show-container', 'style'), Output('hide-container', 'style')],
